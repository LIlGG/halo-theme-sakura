...
function addLineNumbersBlockFor (inputHtml, options) {
  ...
  if (lines.length > 1 || options.singleLine) {
    ...
    for (let i = 0, l = lines.length; i < l; i++) {
      html += `
        <tr>
        <td class="${LINE_NAME} ${NUMBERS_BLOCK_NAME}" ${DATA_ATTR_NAME}="${i + options.startFrom}">
        <div class="${NUMBER_LINE_NAME}" ${DATA_ATTR_NAME}="${i + options.startFrom}"></div>
        </td>
        <td class="${LINE_NAME} ${CODE_BLOCK_NAME}" ${DATA_ATTR_NAME}="${i + options.startFrom}">
