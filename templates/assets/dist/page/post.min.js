import { d as documentFunction, _ as __vitePreload, s as sakura, U as Util } from "../main.min.js";
...
class Post {
  ...
  registerWordCountToast() {
    ...
    if (postWordCount > 0) {
      ...
      let type = "NORMAL";
      if (seconds <= 60 * 10) {
        remind = (sakura.getThemeConfig("post").getValue("post_word_count_toast_normal", String) || "文章篇幅适中，可以放心阅读。").toString();
        type = "NORMAL";
      } else if (seconds > 60 * 10 && seconds <= 60 * 30) {
        remind = (sakura.getThemeConfig("post").getValue("post_word_count_toast_medium", String) || "文章比较长，建议分段阅读。").toString();
        type = "MEDIUM";
      } else {
        remind = (sakura.getThemeConfig("post").getValue("post_word_count_toast_difficulty", String) || "文章内容已经很陈旧了，也许不再适用！").toString();
        type = "DIFFICULTY";
      }
  ...
  registerEditTimeToast(pageConfig) {
    ...
    if (time <= 1e3 * 60 * 60 * 24 * 30) {
      remind = (sakura.getThemeConfig("post").getValue("post_edit_time_toast_normal", String) || "近期有所更新，请放心阅读！").toString();
      type = "NORMAL";
    } else if (time > 1e3 * 60 * 60 * 24 * 30 && time <= 1e3 * 60 * 60 * 24 * 90) {
      remind = (sakura.getThemeConfig("post").getValue("post_edit_time_toast_medium", String) || "文章内容已经有一段时间没有更新了，也许不再适用！").toString();
      type = "MEDIUM";
    } else {
      remind = (sakura.getThemeConfig("post").getValue("post_edit_time_toast_difficulty", String) || "文章内容已经很陈旧了，也许不再适用！").toString();
      type = "DIFFICULTY";
    }
    if (window.innerWidth <= 860) {
      remind = "";
